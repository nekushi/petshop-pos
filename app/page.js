...
export default function LoginPage() {
    const [ currProfile, setCurrProfile ] = useState(userTypes[0].user)
    ...
    return (
        <div className="bg-newRed h-screen overflow-hidden text-white">
            ...
            <Image 
                ...
                height={900}
                width={900}
                className="fixed -bottom-44 left-1/2 -translate-x-1/2 pointer-events-none"
            />
            <Users onChangeProfile={handleChangeProfile} />
        </div>
    )
}
...
function Form() {
    return (
        <form action="" className="w-full text-center space-y-8">
            <label htmlFor="">
                <input 
                    type="text" 
...
function Users({ onChangeProfile }) {
    return (
        <div className="absolute bottom-16 left-16 space-y-4">
            {userTypes.map(user => (
                ...
                >
                    <SmallerIcon />
                    <p
                        onClick={() => {onChangeProfile(user.user)}}
